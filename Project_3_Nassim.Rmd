...
```{r}
plot(Fatalities$fatal ~ Fatalities$year * Fatalities$jail)
```


```{r}
plot(Fatalities$fatal ~ Fatalities$jail)
```

```{r}
hist(Fatalities$fatal) $ SkewedDistributionUtils
```

```{r}
hist(Fatalities$fatal[Fatalities$year==1983])
```

```{r}
interaction.plot(Fatalities$year, Fatalities$jail, Fatalities$fatal)
```
